  ...
  const fetchCamps = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const upcoming = allCamps.filter((camp) => camp.status === 'planned')
        const ongoing = allCamps.filter((camp) => camp.status === 'ongoing')
        const completed = allCamps.filter((camp) => camp.status === 'completed')
        const cancelled = allCamps.filter((camp) => camp.status === 'cancelled')

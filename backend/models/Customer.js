...
class Customer {
    ...
    try {
      const query = `
        SELECT c.*, u.name as employee_name, u.email as employee_email,
               CASE WHEN MAX(card.id) IS NOT NULL THEN true ELSE false END as has_card,
               CASE WHEN MAX(claim.id) IS NOT NULL THEN true ELSE false END as has_claims
        FROM customers c
  ...
  static async getUniqueWorkTypes() {
    try {
      const query = `
        SELECT DISTINCT TRIM(type_of_work) as type_of_work
        FROM customers
        WHERE type_of_work IS NOT NULL 
        AND TRIM(type_of_work) != ''
        ORDER BY type_of_work
      `;
      const result = await db.query(query);
      
      // Process to remove case-insensitive duplicates
      const uniqueTypes = [];
      const seenLowerCase = new Set();
      
      for (const row of result.rows) {
        const trimmed = row.type_of_work.trim();
        const lowerCase = trimmed.toLowerCase();
        
        if (!seenLowerCase.has(lowerCase)) {
          seenLowerCase.add(lowerCase);
          uniqueTypes.push(trimmed);
        }
      }
      
      return uniqueTypes;
    } catch (error) {
      console.error('Error getting unique work types:', error);
      throw error;
    }
  }
}
